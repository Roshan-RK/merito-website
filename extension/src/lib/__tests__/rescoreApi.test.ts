...
describe("rescoreCandidate", () => {
  ...
  it("posts to the rescore endpoint with the key header, url, and jdText", async () => {
    fetchMock.mockResolvedValue({ ok: true, json: async () => ({ fitment: null }) });
    const { rescoreCandidate } = await importRescoreApi();
    await rescoreCandidate("https://www.linkedin.com/in/jane-doe", "We need a backend engineer.");
    expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        body: JSON.stringify({ linkedinUrl: "https://www.linkedin.com/in/jane-doe", jdText: "We need a backend engineer." }),
      })
    );
  });
  ...
  it("returns null on a non-ok response", async () => {
    ...
    const { rescoreCandidate } = await importRescoreApi();
    const result = await rescoreCandidate("https://www.linkedin.com/in/jane-doe", "JD");
