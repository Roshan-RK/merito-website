...
3. **Unresolved — real blocker, not a code bug:** jobs are created **inactive** by default; sending `status: "ACTIVE"` in the create-job body does **not** override this (confirmed: request echoed back `"status":"INACTIVE"` regardless). No API-only fix found. Candidates cannot apply to an inactive job (`"Job is not active"` on `addApplicant`) until it's manually toggled Active in the dashboard. Flagged to IntervueBox; workaround for now is manual toggle per job, which does not scale to the "one job per submission, fully automated" design.
