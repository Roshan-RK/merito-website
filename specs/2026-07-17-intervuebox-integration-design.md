...
**Update (2026-07-21, IntervueBox — Krupal Patel):** Krupal confirmed the JD-only auto-populate feature ships "by today evening." Verified via direct API call earlier the same day that it wasn't live yet (`POST /public/jobs` with only `jobDescription` still 400s, requiring all the other fields). **Still not live as of this update — do not change `createJob` until re-verified live.** Once Krupal confirms it's shipped, re-test with a JD-only body before touching `lib/intervuebox/jobs.ts`.
...
## Live-testing findings (2026-07-21, first real API key)

First real credentials obtained; live-tested the Phase A chain directly (curl + through the app) rather than only against docs. Found several real defects/gaps no amount of doc-reading could have caught — all fixed in code except where noted:

1. **Real working host is `api-in.intervuebox.ai`, not documented `api.intervuebox.ai`.** The documented production host returned `{"message":"Invalid API key","error":"Unauthorized","statusCode":401}` for a key that worked fine against `api-in.intervuebox.ai`. Per the user (2026-07-21): `api-in.intervuebox.ai` is for testing/staging; production will shift to `api.intervuebox.ai` once ready. `INTERVUEBOX_BASE_URL` is already an env var, so no code change needed — just set correctly per environment.
2. **FIXED:** `createJob` needs `skills: []`, `education: []`, `experience: "Not specified"` in the request body — omitting them (as the docs' "optional" marking implied was fine) produces a 400 validation error (`"skills must be an array"` etc.) on the real API.
3. **RESOLVED (2026-07-22, IntervueBox vendor-side default change):** jobs were created **inactive** by default; sending `status: "ACTIVE"` in the create-job body did **not** override this (confirmed: request echoed back `"status":"INACTIVE"` regardless). No API-only fix existed at the time. Krupal/IntervueBox changed the server-side default — new jobs now come back ACTIVE automatically. User live-verified. No code change needed in `lib/intervuebox/jobs.ts`; manual per-job dashboard toggle workaround no longer required.
4. **Real architectural finding:** candidate identity (email specifically) is populated by IntervueBox **parsing the uploaded resume file's content**, not from the `name`/`email`/`phoneNumber` fields sent via the API. A resume with no extractable email text produces `"Email is required"` on `addApplicant` regardless of what the API call itself sends. Confirmed by testing with both a synthetic malformed PDF (failed) and a real DOCX (succeeded once parsing completed) — this is now understood behavior, not a defect, but worth keeping in mind for any future test fixtures.
5. **FIXED:** resume parsing is asynchronous — ~40-50s observed for a real DOCX before `addApplicant` will accept it (`"Resume is still being parsed and has no linked candidate yet"` until then). The fitment-check route previously called `addApplicant` immediately after `uploadResume` with no wait, so it reliably failed. Added `addApplicantWithRetry` (`app/api/hub/fitment-check/route.ts`) — retries specifically on that error message, up to 90s total, 8s interval, both overridable via env (`RESUME_PARSE_MAX_WAIT_MS`, `RESUME_PARSE_RETRY_INTERVAL_MS`) so tests can use near-instant real delays instead of fake timers.
6. **FIXED:** `lib/intervuebox/client.ts`'s error parsing only read the documented nested `{error:{code,message,status}}` envelope. Real error responses are sometimes a flat `{message, error, statusCode}` shape (confirmed live) — the real message text (e.g. "still being parsed") was being discarded into a generic fallback string, which is what hid it from the retry-detection logic in #5 until this was fixed too.
7. **New data point, resolves Open Item #5 above:** resume-match scoring took over 2 minutes and was still `PENDING` when polling stopped — confirms the "minutes+" case from this doc's own decision criteria. Recommendation (per user, 2026-07-21: "lets keep the live polling for now") — no UX change made yet; revisit if 2+ minute in-page spinners prove to be a problem in practice.
8. Rate limit key currently set to the default (100 req/min); per the create-key dialog, can go up to 10,000 req/min if needed later.
9. **RESOLVED — webhook signature verification confirmed correct.** Tested via ngrok tunnel + the dashboard's "Test" button against our real `/api/webhooks/intervuebox` endpoint. The dashboard's "we send this in X-IB-Secret" copy was misleading — it actually sends **both** `X-IB-Secret` (raw) and `X-IB-Signature: t=...,v1=...` (HMAC-SHA256, exactly as documented) plus `X-IB-Timestamp`. Independently recomputed the HMAC by hand against the real captured secret/timestamp/body and it matched byte-for-byte. Our existing `verifySignature()` code (already HMAC-based, written straight from the docs) needed zero changes — real request through the real (non-bypassed) code path returned `200`. This was the last unresolved item from the original design; **Open Item #2 (webhook payload body shape) is now fully closed** for the signing/verification half. The event *body*'s full real shape (`interviewId`/`candidateId`/report fields) is still unconfirmed — the Test button's payload only contains `{test, message, endpointId, event:{eventType, occurredAt, organizationName, organizationId, hrUserId, resellerPath}}`, no interview-specific fields — needs a genuine completed interview to see the rest. Doesn't block anything: the webhook handler is body-shape-agnostic by design (sweep-based, see Architecture section above).
10. **FIXED (partial) — real race between our retry logic and IntervueBox's own async processing.** `addApplicantWithRetry` (#5 above) retries on "still being parsed", but IntervueBox can finish linking the applicant asynchronously *after* returning that error and *before* our retry lands — the retry then hits `"Candidate already applied for this job"` (a plain 400, not 409) instead of succeeding. Same failure shape also seen from `uploadResume` as `"An applied job has already been created for this resume and job."`. Reproduced live with fully unique job/resume/email/phone each time via a standalone script mirroring the retry loop exactly — confirms it's not test-data collision. **Fix shipped:** `isDuplicateApplicantError()` in `app/api/hub/fitment-check/route.ts` now matches these messages by text (not status code) and routes them through the existing friendly duplicate response (`{error: "...sign in...", duplicate: true}`, 409) instead of a dead-end 502 that would just repeat forever on retry. **Still open:** this doesn't recover the actual score for the affected submission — no `fitment_leads` row gets written, and there's no documented endpoint to look up an existing applicant by `jobId`+`resumeId` to fetch the resume-match report after the fact. Flagged to IntervueBox as a feature request (lookup-by-resume endpoint); until then, a user who hits this exact race sees "you've already checked this — sign in" but signing in won't yet surface a report for it.

11. **NEW — jobs-created-inactive blocker RESOLVED (2026-07-22, vendor-side default change).** Krupal/IntervueBox changed the server-side default; new jobs now come back `ACTIVE` automatically. Live-verified via the real signed-in account (`roshan@merito.in`) dashboard flow (`ProgressRail` → `InterviewStartModal` → `start-ai-interview` → `status: "invited"`). No code change needed in `lib/intervuebox/jobs.ts`.
12. **NEW — candidate identity confusion, not a code bug.** IntervueBox derives candidate name/email from the **uploaded resume file's parsed content**, never from the account/session email. A dashboard-triggered interview invite for a lead whose original CV said "Rohan Sharma <rohan.sharma.pm@gmail.com>" emails that address, not the logged-in Merito account's real email — confirmed via `GET /public/applicants/:appliedJobId` showing `candidateEmail: "rohan.sharma.pm@gmail.com"` despite the Merito account being `roshan@merito.in`. Re-confirms live-testing finding #4 above from the dashboard-flow side, not just the anonymous landing flow.
13. **NEW — `rescore-role` missing `phone`, real bug, FIXED.** `app/hub/account/ChangeRoleModal.tsx`'s "Change target role" reupload flow posts `role`/`jdText`/`cv` to `/api/hub/rescore-role`, which forwards to `/api/hub/fitment-check` — but never set `phone`, and `fitment-check` requires it (400 `"A phone number is required."`). The modal itself has no phone field. **Fix shipped (2026-07-22):** `app/api/hub/rescore-role/route.ts` now looks up the user's most recent `fitment_leads.phone` (column added in migration `0011_fitment_leads_phone.sql`) and forwards it automatically — no new UI field needed, matches the modal's "your completed steps carry over" promise.
14. **NEW — IntervueBox dedupes both jobs and resumes by content, undocumented, still open.** `createJob` called twice with the identical `title`+`jobDescription` returns the **same** `jobId` both times rather than creating a fresh job — confirmed live (`JB-6ohtUr7` returned on both calls). Similarly `uploadResume` dedupes by file-content hash: re-uploading a CV whose bytes IntervueBox has already seen returns `400 "Resume already exists"` (no `jobId`) or a confusing `400 "Invalid Resume ID"` (with a *different* `jobId`) instead of cleanly returning the existing `resumeId`. This is broader than the previously-documented resume/applicant duplicate-race (finding #10) — it's content-based dedup on the job itself, not just the resume-to-job link. Practical effect: re-testing the "Change target role" reupload flow with the same role+JD text repeatedly gets stuck reusing the same job/resume records and hitting this dedup wall, even with a fresh CV file. **Workaround used during testing:** tweak the JD text (not just the CV) to force a genuinely new job. **Not yet flagged to IntervueBox** — worth raising alongside the existing lookup-by-resume feature request (#10), since both stem from the same underlying content-dedup design.
