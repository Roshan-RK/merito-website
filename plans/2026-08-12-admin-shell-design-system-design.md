...
- **C** — backend correctness: counselling status-change TOCTOU race, interview generate/reinvite idempotency (incl. unguarded flag-write on failure), `RAZORPAY_BYPASS` interview-start tracking, structured API error responses, swallowed-error logging (`adminCandidates.ts` IntervueBox `.catch(() => null)`), zero test coverage on `adminExtension.ts`/`adminLearnedSkills.ts`/admin share-link path/DB-query fns, `listCandidates()` unbounded scan + N+1 IntervueBox call, minor validation (`notes` field has no `.max()`). Not a UX question, doesn't need this component library.
