...
export default function InterviewOverrideForm({
  interviewRowId,
  overallScore,
  overallSummary,
  overridden,
  ...
  const [text, setText] = useState(overallSummary);
  ...
  async function save() {
    ...
    try {
      const response = await fetch(`/api/admin/interviews/${interviewRowId}/report-override`, {
        ...
        body: JSON.stringify({ overallScore: parsed, overallSummary: text, reason: reason.trim() }),
      });
      ...
      {open && (
        <div style={{ display: "flex", flexDirection: "column", gap: 10, maxWidth: 480, marginTop: 10 }}>
          <label className="font-[family-name:var(--font-poppins)]" style={{ fontSize: 12, display: "flex", flexDirection: "column", gap: 4 }}>
            Overall score (0-10)
            <input type="number" min={0} max={10} step={0.1} value={score} onChange={(e) => setScore(e.target.value)} style={inputStyle} />
          </label>
          <label className="font-[family-name:var(--font-poppins)]" style={{ fontSize: 12, display: "flex", flexDirection: "column", gap: 4 }}>
            Summary
