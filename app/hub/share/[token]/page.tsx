...
import { remapBand, remapBandDark } from "../../account/combined-report/combinedBandColors";
import CombinedCategoryCard from "../../account/combined-report/CombinedCategoryCard";
import CombinedCriteriaCard from "../../account/combined-report/CombinedCriteriaCard";
import CombinedSkillTable from "../../account/combined-report/CombinedSkillTable";
import CombinedRoadmapTimeline from "../../account/combined-report/CombinedRoadmapTimeline";
import CondensedPersonalitySection from "../../account/combined-report/CondensedPersonalitySection";
import { InlineText } from "../../account/EvaluatorNotes";
...
import AnimatedDimensionBars from "./AnimatedDimensionBars";
import PublicReportRail from "./PublicReportRail";
...
function UnavailableState() {
  return (
    <main
      ...
      style={{ background: "#F4F1F7", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}
    >
      <div className="bg-white border border-black/[0.08]" style={{ borderRadius: 22, padding: "40px 36px", maxWidth: 420, textAlign: "center" }}>
        <h1 className="font-[family-name:var(--font-fraunces)] font-semibold text-black" style={{ fontSize: "1.4rem", margin: "0 0 10px" }}>
          This link is no longer available
        </h1>
        <p className="font-[family-name:var(--font-inter)] text-[#6C6779]" style={{ fontSize: 14, lineHeight: 1.6, margin: 0 }}>
          The candidate may have revoked this report link, or it doesn&apos;t exist. Ask them to share a fresh link.
        </p>
      </div>
    </main>
  );
...
export default async function PublicSharePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const interviewSections = new Set(validation.interviewSections);
  ...
  const { fitment, personality, interview, references, displayName, primaryRole } = await loadCombinedReportData({
    ...
    userEmail: undefined,
    include,
    roleTitleParam: validation.roleTitle,
  });
  ...
  const reportId = `MH-${validation.userId.split("-")[0].toUpperCase()}`;
  ...
  return (
    <main className={`${fraunces.variable} ${inter.variable} ${ibmPlexMono.variable}`} style={{ background: "#F4F1F7", minHeight: "100vh" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0,1fr))", gap: 14 }}>
            {fitment && fitmentBand && fitmentBandDark && (
              <div className="bg-white/[0.04] border border-white/[0.1]" style={{ borderRadius: 16, padding: "18px 14px 16px", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", gap: 10 }}>
                <AnimatedGauge value={fitment.report.overallScore} max={100} displayValue={`${Math.round(fitment.report.overallScore)}%`} diameter={96} band={fitmentBandDark} numberColor="#fff" />
            ...
            {interview && interviewBand && interviewBandDark && (
              <div className="bg-white/[0.04] border border-white/[0.1]" style={{ borderRadius: 16, padding: "18px 14px 16px", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", gap: 10 }}>
                <AnimatedGauge value={interview.report.overallScore} max={100} displayValue={`${Math.round(interview.report.overallScore)}%`} diameter={96} band={interviewBandDark} numberColor="#fff" />
            ...
            {references && refBand && (
              <div className="bg-white/[0.04] border border-white/[0.1]" style={{ borderRadius: 16, padding: "18px 14px 16px", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", gap: 10 }}>
                <AnimatedGauge value={references.overallScore} max={5} displayValue={references.overallScore.toFixed(1)} diameter={96} band={{ textColor: refBand.textColor === "#1E8F5E" ? "#3FCB8C" : refBand.textColor === "#95324B" ? "#E8798F" : "#BD7E12", trackColor: "rgba(255,255,255,0.12)" }} numberColor="#fff" />
            ...
            {personality && (
              <div className="bg-white/[0.04] border border-white/[0.1]" style={{ borderRadius: 16, padding: "18px 14px 16px", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", gap: 10 }}>
                <AnimatedGauge value={5} max={5} displayValue="5/5" diameter={96} band={{ textColor: "#9C97E8", trackColor: "rgba(255,255,255,0.12)" }} numberColor="#fff" />
          ...
          {fitment && fitmentBand && (
            <div id="fitment">
              ...
              <SummaryRow
                ...
                gauge={<AnimatedGauge value={fitment.report.overallScore} max={100} displayValue={`${Math.round(fitment.report.overallScore)}%`} caption="Overall match" diameter={150} band={fitmentBand} />}
              >
                {fitment.report.summary}
              </SummaryRow>
              <AnimatedDimensionBars
              ...
              {interviewSections.has("scoreGauge") && (
                <>
                  <SummaryRow
                    ...
                    gauge={<AnimatedGauge value={interview.report.overallScore} max={100} displayValue={`${Math.round(interview.report.overallScore)}%`} caption="Overall score" diameter={150} band={interviewBand} />}
                  >
          ...
          {references && refBand && (
            <div id="references">
              ...
              <SummaryRow
                ...
                gauge={<AnimatedGauge value={references.overallScore} max={5} displayValue={references.overallScore.toFixed(1)} caption="Out of 5" diameter={150} band={refBand} />}
              >
