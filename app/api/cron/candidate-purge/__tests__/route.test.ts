...
vi.mock("@/lib/purgeCandidates", () => ({ purgeDueCandidateDeletions: purgeMock }));
...
function request(auth?: string) {
  return new Request("http://localhost/api/cron/candidate-purge", {
    headers: auth ? { authorization: auth } : {},
  });
}

describe("GET /api/cron/candidate-purge", () => {
  beforeEach(() => {
  ...
  it("returns the purged count on success", async () => {
    purgeMock.mockResolvedValue({ purgedCount: 2 });
    const { GET } = await importRoute();
    const response = await GET(request("Bearer secret-123"));
    const body = await response.json();
    expect(response.status).toBe(200);
    expect(body).toEqual({ purgedCount: 2 });
  });
});
